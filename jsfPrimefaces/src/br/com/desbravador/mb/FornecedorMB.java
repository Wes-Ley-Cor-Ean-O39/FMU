...
import br.com.desbravador.dao.FornecedorDao;
...
public class FornecedorMB {
	...
	public void excluirFornecedor(ActionEvent actionEvent) {
		...
		consultarAllFornecedor();
		addMessage("Fornecedor Exclu�do com Sucesso!");
	}
	...
	public void buscaCep(AjaxBehaviorEvent event) {
		Document doc;
		try {
			doc = Jsoup.connect("http://cep.republicavirtual.com.br/web_cep.php?cep="
					+ fornecedor.getCep().replaceAll("-", "")).get();
			Elements links = doc.getAllElements();
			fornecedor.setEndereco(links.get(11).text());
			fornecedor.setCidade(links.get(8).text());
			fornecedor.setBairro(links.get(9).text());
	...
	public void excluirFornecedorList(ActionEvent event) {
		...
		consultarAllFornecedor();
		addMessage("Fornecedor(es) Exclu�do(s) com Sucesso!");
	}
